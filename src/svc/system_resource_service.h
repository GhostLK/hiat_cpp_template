...
public:
     ...
     */
    bool allocate_cpu_cores(std::initializer_list<int> cpu_ids) {
...
public:
     ...
     */
    bool allocate_cpu_cores(std::initializer_list<int> cpu_ids) {
        std::unique_lock<std::shared_mutex> lock(resource_mutex_);
    ...
    bool is_cpu_allocated(int cpu_id) {
        ...
        std::shared_lock<std::shared_mutex> lock(resource_mutex_);
...
public:
    ...
    bool allocate_cpu_cores(std::initializer_list<int> cpu_ids) {  // NOLINT
        (void)cpu_ids;
        return true;
    }
...
#define SX_ALLOCATE_CPU_CORES(cpu_ids) \
    ::sx::LinuxResourceManager::get_instance().allocate_cpu_cores(cpu_ids)
